...
export const TerminalSidebar: React.FC = () => {
  const { terminals, activeTerminalId, selectTerminal, removeTerminal } =
    useTerminalContext();
    ...
    <div className='bg-[#252526] text-[#cccccc] w-48'>
      <div className='py-1'>
        {terminals.map((terminal) => (
          <div
            className={`flex items-center justify-center hover:bg-[#ffffff1a]  ${
              activeTerminalId === terminal.id ? "bg-[#37373d]" : ""
            }`}
          >
            <button
              key={terminal.id}
